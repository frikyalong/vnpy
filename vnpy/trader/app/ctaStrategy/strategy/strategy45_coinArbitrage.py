...
#----------------------------------------------------------------------
    def __init__(self, cmaEngine, setting=None):
        ...
        self.paramList.append('ratio_upper')
        self.paramList.append('ratio_lower')
        ...
        self.ratio_upper = 1.001            # 价比上阈值
        self.ratio_lower = 0.999            # 价比下阈值
        ...
        if orderkey in self.uncompletedOrders:
            if order.totalVolume == order.tradedVolume:
                ...
                self.__onOrderAllTraded(order)
                    ...
                    and not order.totalVolume == order.tradedVolume and\
                    order.status not in [STATUS_CANCELLED,STATUS_REJECTED]:
                # 委托单部分成交
    ...
    def buy(self, volume):
        ...
        if (self.master_quote_pos.longPosition-self.master_quote_pos.frozen) / self.last_master_tick.lastPrice < volume:
            self.writeCtaLog(u'{}市场货币:{}不足买入:{} {}'
                             .format(self.master_gateway,self.quote_symbol, volume, self.base_symbol))
            return False
    ...
    def sell(self, volume):
        ...
        if (self.slave_quote_pos.longPosition  - self.slave_quote_pos.frozen)/ self.last_slave_tick.lastPrice < volume:
            self.writeCtaLog(u'{}市场货币:{}不足买入:{} {}'
                             .format(self.slave_gateway, self.quote_symbol, volume, self.base_symbol))
            return False
    ...
    def onTick(self, tick):
        ...
        if not (self.inited and \
                len(self.lineDiff.lineMiddleBand) > 0 \
                and len(self.lineRatio.lineStateMean) > 0 \
                and len(self.lineMD.lineMiddleBand) > 0 \
                and len(self.lineM5.lineUpperBand)>0) :
            return
        ...
        if spread_tick.bidPrice1 > self.lineDiff.lineUpperBand[-1]\
            ...
            if self.master_entrust == 0 and self.slave_entrust == 0 \
                    and len(self.uncompletedOrders)==0 and len(self.policy.uncomplete_orders) ==0:
        ...
        if spread_tick.askPrice1 < self.lineDiff.lineLowerBand[-1] \
                ...
                and ratio_tick.bidPrice1 <= self.ratio_lower:
            self.writeCtaLog(u'Buy Signal:{}, buy master:{}/{}/{}, sell slave:{}/{}/{}'
                             .format(spread_tick.askPrice1,
                                     self.last_master_tick.askPrice1, self.last_master_tick.lastPrice, self.last_master_tick.bidPrice1,
                                     self.last_slave_tick.askPrice1, self.last_slave_tick.lastPrice, self.last_slave_tick.bidPrice1))
            # 当前没有委托，没有未完成的订单，没有重新激活的订单
            if self.master_entrust == 0 and self.slave_entrust == 0 and \
                    len(self.uncompletedOrders)==0 and len(self.policy.uncomplete_orders) ==0:
    ...
    def onBarM5(self, bar):
        ...
        if self.master_entrust == 0 and self.slave_entrust == 0 and len(self.uncompletedOrders) == 0 and len(self.policy.uncomplete_orders) > 0:
            self.writeCtaLog(u'存在未执行的策略{}，重新执行'.format(self.policy.uncomplete_orders))
            self.resumbit_orders()
