...
#
        ...
        self.lineM = None                      # 5分钟K线

        self.logMsg = EMPTY_STRING              # 临时输出日志变量

        self.df = None                          # dataframe

        self.mean_short_signal = False
        self.mean_buy_signal = False
        self.mean_cover_signal = False
        self.mean_sell_signal = False

        self.last_mean_bar = None               # 上一根MeanBar
        ...
        if setting:
            ...
            lineMSetting['barTimeInterval'] = 60
            lineMSetting['inputBollLen'] = 60
            lineMSetting['inputBollStdRate'] = 3
    ...
    def __InitM1FromCsv(self):
        try:
            self.df = pd.read_csv('cache/M1609_M1701_20160401_20160830.csv')
            if self.df is None:
                return
        except:
            return

        self.df = self.df.rename(columns={'Unnamed: 0': 'datetime'})
        self.df = self.df.set_index(['datetime'])

    def __getM1(self,barTime):
        if self.df is None:
            return None
        try:
            bar = self.df.ix[barTime.strftime('%Y-%m-%d %H:%M:%S')]
            if len(bar) < 1:
                return None
            return bar
    ...
    def onTick(self, tick):
        ...
        # 合约已计算好价差，无需在k线内计算价差,直接推送Tick到lineM中即可。    
        self.lineM.onTick(tick)

        # 4、交易逻辑

        # 首先检查是否是实盘运行还是数据预处理阶段
        if not (self.inited and len(self.lineM.lineMiddleBand) > 0):
            return
        ...
        if len(self.gt.upGrids) <= 0 or len(self.gt.dnGrids) <= 0:
            ...
            upper = round(self.lineM.lineUpperBand[-1],2)           
            upper = upper - upper % self.minDiff

            lower = round(self.lineM.lineLowerBand[-1], 2)            
        ...
        self.__verifyMeanBar(tick)

        # 价差与本地挂单最接近价格时，激活本地挂单，提交至服务器，若价差离开挂单太多时，服务器取消挂单

        # 价差进入做空区域；价差高于布林下轨；价差不是当前最高点时，开始挂反套单
        #if tick.bidPrice1 >= max(self.baseUpLine, self.lineM.lastBollMiddle) \
        #       and tick.askPrice1 < self.lineM.lineBar[-1].high \
        if self.tradingOpen \
                and self.mean_short_signal:
            ...
            m1Up = round(self.lineM.lineUpperBand[-1], 2)
            ...
            for x in pendingGrids[:]:
                ...
                if openFlag and self.position.avaliablePos2Add()>0:
                    ...
                    if self.position.avaliablePos2Add()> 0:
                        msg = u'网格[{0}]< M1:{1}，不挂单'.format(x.openPrice, m1Up)
                    else:
                        msg = u'持空仓数量已满，不再开仓'
                    if msg != self.logMsg:
                        self.logMsg = msg
        ...
        # if tick.askPrice1 < min(self.baseDnLine, self.lineM.lastBollMiddle) \
        #        and tick.bidPrice1 > self.lineM.lineBar[-1].low \
        if self.tradingOpen\
                and self.mean_buy_signal:

            # 获取价格接近的未挂多单（当前价格 +- 1跳）
            pendingGrids = self.gt.getGrids(direction=DIRECTION_LONG, begin=tick.askPrice1+self.minDiff,
                                            end=tick.askPrice1-self.minDiff)

            m1Dn= round(self.lineM.lineLowerBand[-1], 2)
            ...
            for x in pendingGrids:
                ...
                if openFlag and self.position.avaliablePos2Add()>0:
                    ...
                    if self.position.avaliablePos2Add() > 0:
                        msg = u'网格[{0}]> M1:{1}，不挂单'.format(x.openPrice, m1Dn)
                    else:
                        msg = u'持多仓数量已满，不再开仓'
    ...
    def onBar(self, bar):
        ...
        if len(self.lineM.lineUpperBand) > 0:
            upper = round(self.lineM.lineUpperBand[-1], 2)
        else:
            upper = 0

        if len(self.lineM.lineMiddleBand) > 0:
            middle = round(self.lineM.lineMiddleBand[-1], 2)
        else:
            middle = 0

        if len(self.lineM.lineLowerBand) > 0:
            lower = round(self.lineM.lineLowerBand[-1], 2)
        else:
            lower = 0
        ...
        mean_bar = self.__getM1(bar.datetime + timedelta(seconds=self.lineM.barTimeInterval))
        if mean_bar is None:  return
        self.last_mean_bar = mean_bar.to_dict()
        self.lineM.displayLastBar()
        self.writeCtaLog(u'{0}'.format(self.last_mean_bar))


    def __verifyMeanBar(self, tick):

        if self.last_mean_bar is None: return

        leg1_leg2_dif = float(self.last_mean_bar['leg1_leg2_dif'])    # 价差
        mean_leg2_dif = float(self.last_mean_bar['mean_leg2_dif'])    # 残差
        boll_upper = float(self.last_mean_bar['boll_upper'])            # 残差boll上轨
        boll_lower = float(self.last_mean_bar['boll_lower'])            # 残差boll下轨
        boll_mid = float(self.last_mean_bar['boll_mid'])                # 残差boll中轨
        boll_std = (boll_upper - boll_lower)/4                           # 标准差

        if mean_leg2_dif > boll_upper and boll_std > 3*self.minDiff and leg1_leg2_dif > 0 and boll_mid > 0:
            self.writeCtaLog(u'mean适合开空')
            self.mean_short_signal = True
            self.mean_buy_signal = False
            return

        if mean_leg2_dif < boll_lower and boll_std > 3*self.minDiff and leg1_leg2_dif < 0 and boll_mid < 0:
            self.writeCtaLog(u'mean适合开多')
            self.mean_buy_signal = True
            self.mean_short_signal = False
            return

        if self.mean_short_signal and mean_leg2_dif < 0:
            self.writeCtaLog(u'Mean平空')
            self.mean_cover_signal = True
            self.mean_short_signal = False

            self.__closeAllGrids(direction=DIRECTION_SHORT, closePrice=self.curTick.askPrice1)

        if self.mean_buy_signal and mean_leg2_dif > 0:
            self.writeCtaLog(u'Mean平多')
            self.mean_buy_signal = False
            self.mean_sell_signal = True
            self.__closeAllGrids(direction=DIRECTION_LONG, closePrice=self.curTick.bidPrice1)
    ...
    def __closeAllGrids(self, direction, closePrice):
        ...
        if direction == DIRECTION_SHORT:
            ...
            for x in self.gt.upGrids[:]:
                # 已发送订单，已开仓，未平仓
                if not x.openStatus or not x.orderStatus or x.closeStatus or x.orderRef == EMPTY_STRING:
                    continue

                self.writeCtaLog(u'取消平空单:[ref={0},closeprice={1}]'.format(x.orderRef, x.closePrice))
                self.cancelOrder(x.orderRef)
                ...
                ref = self.cover(price=x.closePrice, volume=x.volume - x.tradedVolume)

                if ref:
                    x.orderRef = ref
                    self.writeCtaLog(u'强制提交平空委托单[closeprice={0},volume={1}]'
                                     .format(x.closePrice, x.volume - x.tradedVolume))
                else:
        ...
        if direction == DIRECTION_LONG:
            ...
            for x in self.gt.dnGrids[:]:
                ...
                ref = self.sell(price=x.closePrice, volume=x.volume - x.tradedVolume)

                if ref:
                    x.orderRef = ref
                    self.writeCtaLog(u'强制提交平多委托单[closeprice={0},volume={1}]'.format(x.closePrice, x.volume - x.tradedVolume))
                else:
                    self.writeCtaLog(u'提交平仓委托单失败')
                    x.closePrice = oldPrice
    ...
    def __resubmitFirstGrid(self, direction, lastVolume):
        ...
        if direction == DIRECTION_LONG:
            ...
            ref = self.sell(price=grid.closePrice, volume=grid.volume - grid.tradedVolume)

            if ref:
                grid.orderRef = ref
                self.writeCtaLog(u'提交平仓委托单[closeprice={0},volume={1}]'.format(grid.closePrice,grid.volume - grid.tradedVolume))
            else:
                self.writeCtaLog(u'提交平仓委托单失败')
                grid.closePrice = oldPrice
...
def testM():
    ...
    settings['baseUpLine'] = 0
    settings['baseDnLine'] = 0
    settings['height'] = 4
    settings['win'] = 4
    settings['maxPos'] = 80
    settings['maxLots'] = 30
    ...
    if p == 'Windows':
        ...
        leg2path = '/home/ubuntu/Ticks/DL'

    engine.runBackTestingWithArbTickFile2(leg1path, leg2path, 'SP M1709&M1801')
...
if __name__ == '__main__':
    ...
    setup_logger(
        filename=u'{}/TestLogs/{}_{}.log'.format(cta_engine_path,Strategy_ArbitrageGrid.className, datetime.now().strftime('%m%d_%H%M')),
        debug=False)
