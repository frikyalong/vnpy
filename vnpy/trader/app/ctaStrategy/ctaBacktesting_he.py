...
class BacktestingEngine(object):
    ...
    def __loadArbTicks2(self, leg1MainPath, leg2MainPath, testday, leg1Symbol, leg2Symbol):
        ...
        # 先读取leg2的数据到目录，以日期时间为key
        leg2Ticks = self.__loadTicksFromCsvFile_44cols(filepath=leg2File, tickDate=testday, vtSymbol=leg2Symbol)

        leg1Ticks = self.__loadTicksFromCsvFile_44cols(filepath=leg1File, tickDate=testday, vtSymbol=leg1Symbol)
    ...
    def __loadTicksFromCsvFile_44cols(self, filePath, tickDate, vtSymbol):
        """
         新版本的数据 - 44个字段
         从csv文件中UnicodeDictReader读取tick
         该函数仅限上期所，因为只有上期所的tick毫秒数据是0或500默认排列好。
         其他交易所的数据，需要对tick的毫秒数据做处理。（请看注释部分的sample）
         """

        if not os.path.isfile(filePath):
            self.writeCtaLog(u'{0}该文件不存在'.format(filePath))
            return

        ticks = OrderedDict()

        dt = None
        print(u'加载{0}'.format(filePath))

        # 为了回测方便，该方法在回测引擎中直接用pandas计算了若干指标，简化了策略中的指标计算
        df = pd.read_csv(filePath, encoding='gbk', parse_dates=False)

        df.columns = ['TradingDay', 'InstrumentID', 'ExchangeID', 'ExchangeInstID', 'LastPrice',
        ...
        # # ---------------添加回测要用到的指标------------------------------------
        # df['tickVolume'] = df['Volume'].diff(1)
        # df['tickTurnover'] = df['Turnover'].diff(1)
        #
        # df['tickSpread'] = df['AskPrice1'] - df['BidPrice1']
        #
        # # tick内的平均成交价，如果该tick内没有交易，但档口量有变化，则这个tick内的成交价为None
        # # 注意要载入正确的self.size
        # df['tickAvgTradePrice'] = (df['tickTurnover'] / self.size) / df['tickVolume']
        # # 将avgPrice为0的数值都替换为它之前的非零数值
        # # df['tickAvgTradePrice_filled'] = df['tickAvgTradePrice'].replace(to_replace=0, method='ffill')
        # # 将avgPrice为None的数值都替换为它之前的非零数值
        # df['tickAvgTradePrice_filled'] = df['tickAvgTradePrice'].fillna(method='ffill')
        #
        # df['tickMidPrice'] = (df['AskPrice1'] + df['BidPrice1']) / 2
        #
        # # Bid-Ask Spread
        # df['index_AskBid_spread'] = df['AskPrice1'] - df['BidPrice1']
        #
        # df['index_MA_5'] = df['LastPrice'].rolling(window=5).mean()  # 指定宽度的滑动窗口,普通均值
        # df['index_MA_10'] = df['LastPrice'].rolling(window=10).mean()
        # df['index_MA_30'] = df['LastPrice'].rolling(window=30).mean()  # 指定宽度的滑动窗口,普通均值
        # # df['index_MA_60'] = df['LastPrice'].rolling(window=60).mean()
        # # df['index_MA_120'] = df['LastPrice'].rolling(window=120).mean()
        # # df['index_MA_240'] = df['LastPrice'].rolling(window=240).mean()
        #
        # # df['index_EMA_10'] = df['LastPrice'].ewm(span=10).mean()
        # # df['index_EMA_20'] = df['LastPrice'].ewm(span=20).mean()
        # # df['index_EMA_30'] = df['LastPrice'].ewm(span=30).mean()
        # # df['index_EMA_60'] = df['LastPrice'].ewm(span=60).mean()
        #
        # self.calc_index_weight_avgPrice(df, 10)  # index_Weight_AvgPrice_10
        # self.calc_index_weight_avgPrice(df, 30)
        # self.calc_index_weight_avgPrice(df, 60)
        # self.calc_index_weight_avgPrice(df, 240)
        # self.calc_index_weight_avgPrice(df, 360)
        # self.calc_index_weight_avgPrice(df, 480)
        # self.calc_index_weight_avgPrice(df, 600)
        # self.calc_index_weight_avgPrice(df, 640)
        # self.calc_index_weight_avgPrice(df, 720)
        # self.calc_index_weight_avgPrice(df, 840)
        # self.calc_index_weight_avgPrice(df, 960)
        # self.calc_index_weight_avgPrice(df, 1080)
        # self.calc_index_weight_avgPrice(df, 1200)
        # self.calc_index_weight_avgPrice(df, 2400)
        # self.calc_index_weight_avgPrice(df, 3600)
        # # self.calc_index_weight_avgPrice(df, 4800)
        # # self.calc_index_weight_avgPrice(df, 6000)
        # # self.calc_index_weight_avgPrice(df, 7200)
        # # self.calc_index_weight_avgPrice(df, 8400)
        # # self.calc_index_weight_avgPrice(df, 14400)
        #
        # df['index_VolMA_5'] = df['tickVolume'].rolling(window=5).mean()
        # df['index_VolMA_10'] = df['tickVolume'].rolling(window=10).mean()
        # df['index_VolMA_30'] = df['tickVolume'].rolling(window=30).mean()
        #
        # df['index_VolEMA_5'] = df['tickVolume'].ewm(span=5).mean()
        # df['index_VolEMA_10'] = df['tickVolume'].ewm(span=10).mean()
        # df['index_VolEMA_30'] = df['tickVolume'].ewm(span=30).mean()
        ...
        for i in range(0, len(df)):
            ...
            tick.vtSymbol = vtSymbol  # vtSymbol通常是Symbol.Exchange，但在此与symbol相同。
            tick.symbol = vtSymbol

            tick.symbol = row['InstrumentID']
            tick.exchange = row['ExchangeID']
            ...
            tick.time = '.'.join([row['UpdateTime'], str(int(row['UpdateMillisec'] / 100))])
            tick.date = str(row['ActionDay'])
            tick.datetime = datetime.strptime(tick.date + ' ' + tick.time, '%Y%m%d %H:%M:%S.%f')

            tick.preOpenInterest = row['PreOpenInterest']  # 昨持仓量
            tick.openInterest = row['OpenInterest']  # 持仓量
            tick.PreClosePrice = row['PreClosePrice']  # 昨日收盘价
            tick.todayAvgAmount = row['AveragePrice']  # 当日的每手平均成交金额，这个价格要除以size才是行情价格 （=成交金额除以成交量）
            tick.turnover = row['Turnover']  # 当日成交总金额

            tick.openPrice = row['OpenPrice']
            tick.highPrice = row['HighestPrice']
            tick.lowPrice = row['LowestPrice']
            tick.preClosePrice = row['PreClosePrice']

            tick.upperLimit = row['UpperLimitPrice']
            tick.lowerLimit = row['LowerLimitPrice']
            ...
            # 原始数据中不提供每个tick中的成交量，需要在策略中自行计算, 不需要在回测程序中计算,回测程序只传送基本数据字段

            # # 为了提高测试速度，将pandas中生成的指标赋值到每个tick结构体中
            # tick.index_MA_5 = row['index_MA_5']
            # tick.index_MA_10 = row['index_MA_10']
            # tick.index_MA_30 = row['index_MA_30']
            # # tick.index_MA_60 = row['index_MA_60']
            # # tick.index_MA_120 = row['index_MA_120']
            # # tick.index_MA_240 = row['index_MA_240']
            # #
            # # tick.index_EMA_10 = row['index_EMA_10']
            # # tick.index_EMA_20 = row['index_EMA_20']
            # # tick.index_EMA_30 = row['index_EMA_30']
            # # tick.index_EMA_60 = row['index_EMA_60']
            #
            # tick.index_Weight_AvgPrice_10 = row['index_Weight_AvgPrice_10']
            # tick.index_Weight_AvgPrice_30 = row['index_Weight_AvgPrice_30']
            # tick.index_Weight_AvgPrice_60 = row['index_Weight_AvgPrice_60']
            # tick.index_Weight_AvgPrice_240 = row['index_Weight_AvgPrice_240']
            # tick.index_Weight_AvgPrice_360 = row['index_Weight_AvgPrice_360']
            # tick.index_Weight_AvgPrice_480 = row['index_Weight_AvgPrice_480']
            # tick.index_Weight_AvgPrice_600 = row['index_Weight_AvgPrice_600']
            # tick.index_Weight_AvgPrice_640 = row['index_Weight_AvgPrice_640']
            # tick.index_Weight_AvgPrice_720 = row['index_Weight_AvgPrice_720']
            # tick.index_Weight_AvgPrice_840 = row['index_Weight_AvgPrice_840']
            # tick.index_Weight_AvgPrice_960 = row['index_Weight_AvgPrice_960']
            # tick.index_Weight_AvgPrice_1080 = row['index_Weight_AvgPrice_1080']
            # tick.index_Weight_AvgPrice_1200 = row['index_Weight_AvgPrice_1200']
            # tick.index_Weight_AvgPrice_2400 = row['index_Weight_AvgPrice_2400']
            # tick.index_Weight_AvgPrice_3600 = row['index_Weight_AvgPrice_3600']
            # # tick.index_Weight_AvgPrice_4800 = row['index_Weight_AvgPrice_4800']
            # # tick.index_Weight_AvgPrice_6000 = row['index_Weight_AvgPrice_6000']
            # # tick.index_Weight_AvgPrice_7200 = row['index_Weight_AvgPrice_7200']
            # # tick.index_Weight_AvgPrice_8400 = row['index_Weight_AvgPrice_8400']
            # # tick.index_Weight_AvgPrice_14400 = row['index_Weight_AvgPrice_14400']
            #
            # tick.todayAveragePrice = row['AveragePrice'] / self.size
            #
            # tick.index_VolMA_5 = row['index_VolMA_5']
            # tick.index_VolMA_10 = row['index_VolMA_10']
            # tick.index_VolMA_30 = row['index_VolMA_30']
            #
            # tick.index_VolEMA_5 = row['index_VolEMA_5']
            # tick.index_VolEMA_10 = row['index_VolEMA_10']
            # tick.index_VolEMA_30 = row['index_VolEMA_30']

            # 排除涨停/跌停的数据
            if (tick.bidPrice1 == float('1.79769E308') or tick.bidVolume1 == 0) \
                    or (tick.askPrice1 == float('1.79769E308') or tick.askVolume1 == 0):
                continue

            dtStr = tick.date + ' ' + tick.time  # Eric - 如果一秒之内有3个tick，这是有可能的，那么会pass掉第三个tick
            if dtStr in ticks:
                pass
    ...
    def __loadNotStdArbTicks2(self, leg1MainPath, leg2MainPath, testday,  leg1Symbol, leg2Symbol):
        ...
        #E:\Ticks\2018\201802\ag1802_20180201.csv
        leg1File = os.path.abspath(
            os.path.join(leg1MainPath, testday.strftime('%Y'), testday.strftime('%Y%m'),
                         '{0}{1}_{2}.csv'.format(leg1_shortSymbol,leg1Symbol[-4:],testday.strftime('%Y%m%d'))))
        ...
        leg2File = os.path.abspath(
            os.path.join(leg2MainPath, testday.strftime('%Y'), testday.strftime('%Y%m'),
                         '{0}{1}_{2}.csv'.format(leg2_shortSymbol, leg2Symbol[-4:], testday.strftime('%Y%m%d'))))
    ...
    def realtimeCalculate(self):
        ...
        for tradeid in tradeids:
            ...
            if trade.volume == EMPTY_INT:
                self.writeCtaLog(
                    u'{},dir:{},vtOrderID:{}tradeID:{}的volumn为{},删除'.format(trade.vtSymbol, trade.direction,
                                                                            trade.vtOrderID, trade.tradeID,
                                                                            trade.volume))
                try:
